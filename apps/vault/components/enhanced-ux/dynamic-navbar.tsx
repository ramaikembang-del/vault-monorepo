...
export function DynamicNavbar() {
    ...
    React.useEffect(() => {
        const checkModes = () => {
            setIsCompact(window.matchMedia("(max-width: 1024px)").matches);
            setIsTouch(window.matchMedia("(pointer: coarse)").matches);

            // Detect navbar direction (row vs column)
            const direction = getComputedStyle(document.documentElement)
                .getPropertyValue("--nav-direction").trim();
            setIsRowOrientation(direction === "row");
        };
        checkModes();
        ...
        >
            ...
            >
                <div
                    ...
                    style={{ height: "var(--nav-icon-box)", width: "var(--nav-icon-box)" }}
                >
                    <svg
            ...
            >
                ...
                {(!isMobile || isExpanded) && (
                    <div
                        className={cn(
                            "group relative flex items-center transition-all hover:bg-accent cursor-pointer",
                            isMobile ? "w-auto" : "w-full"
                        )}
                        style={{
                            height: "var(--nav-item-height)",
